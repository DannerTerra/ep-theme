...
$main_buttons = ['Servidores','Estudantes','Empresas / Organizações'];

$main_buttons = [
				'servidores' => ['link' => '#servidores', 'text' => 'Servidores'], 
				'estudantes' => ['link' => '#estudantes', 'text' => 'Estudantes'], 
				'empresas' => ['link' => '#empresas', 'text' => 'Empresas / Organizações']
				];
// var_dump($main_buttons);
// die();
...
if ( ! empty( $paralax_one_header_logo ) || ! empty( $parallax_one_header_title ) || ! empty( $parallax_one_header_subtitle ) || ! empty( $parallax_one_header_button_text ) ) {
	...
	if ( ! empty( $parallax_one_enable_move ) && $parallax_one_enable_move ) {
		...
		if ( empty( $parallax_one_first_layer ) && empty( $parallax_one_second_layer ) ) {
			...
			echo '<li class="layer layer1" data-depth="0.10" style="background-image: url('. get_site_url() . '/wp-content/themes/Parallax-One/images/fundos/fundo_principal.jpeg);"></li>';
			...
			foreach ($main_buttons as $key) {
							...
							?>
								...
								if ( ! empty( $parallax_one_go_to ) ) {
									...
									echo 'onclick="return false;" data-anchor="' . $key['link'] . '" '; }
?>
 class="btn btn-primary main-button inpage-scroll inpage_scroll_btn">
									<span class="screen-reader-text"><?php echo esc_html__( 'Header button label:','parallax-one' ) . //strip_tags( trim( $parallax_one_header_button_text ) ); 
									strip_tags( trim( $key['text'] ) ); ?></span>
									<?php //echo wp_kses_post( $parallax_one_header_button_text );
									echo wp_kses_post( $key['text'] ); ?>
								</button>
								<?php
							} elseif ( is_customize_preview() ) {
							?>
								<button class="btn btn-primary main-button inpage-scroll inpage_scroll_btn paralax_one_only_customizer"  
								<?php
								if ( ! empty( $parallax_one_go_to ) ) {
									echo $parallax_one_go_to; }
?>
