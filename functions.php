...
if ( ! function_exists( 'parallax_one_setup' ) ) :
	...
	function parallax_one_setup() {
		...
		*/
		...
		add_image_size( 'parallax_one_post-thumbnail-medium', 75, 50, true );
...
/**
Filtro excerpt  for project cards   
*/
function project_cards_tamanho_customizado( $length ) {
    return 25;
}
add_filter( 'excerpt_length', 'project_cards_tamanho_customizado', 999 );

function project_cards_excerpt_more( $more ) {
    return ' [...]';
}
add_filter( 'excerpt_more', 'project_cards_excerpt_more' );
...
function parallax_one_scripts() {
	...
	if ( ('posts' == get_option( 'show_on_front' ) && is_front_page()) || is_page() ) {
